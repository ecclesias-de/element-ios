...
struct AuthenticationEmailServerSelectionScreen: View {
    // MARK: - Properties
    ...
    @ObservedObject var viewModel: AuthenticationEmailServerSelectionViewModel.Context
    ...
    var header: some View {
        VStack(spacing: 8) {
            OnboardingIconImage(image: Asset.Images.authenticationServerSelectionIcon)
                .padding(.bottom, 8) // milan: todo remove delete icon
            
            Text(VectorL10n.authLogin) // milan: todo update text
                .font(theme.fonts.title2B)
                .multilineTextAlignment(.center)
                .foregroundColor(theme.colors.primaryContent)
                .accessibilityIdentifier("headerTitle")
            
            Text(VectorL10n.authenticationVerifyEmailInputTitle) // milan: todo update text
    ...
    var textField: some View {
        TextField(VectorL10n.authenticationVerifyEmailTextFieldPlaceholder, text: $viewModel.email) { // milan: todo update text
        ...
        .onChange(of: viewModel.email) { _ in viewModel.send(viewAction: .clearFooterError) }
...
@available(iOS 15.0, *)
struct AuthenticationEmailServerSelection_Previews: PreviewProvider {
    static let stateRenderer = MockAuthenticationEmailServerSelectionScreenState.stateRenderer
